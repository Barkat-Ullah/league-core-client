...
const footerLinks = {
  ...
  contact: [
    {
      name: "info@league.dev",
      href: "mailto:info@league.dev",
      icon: Mail,
    },
    { name: "0199999999", href: "tel:0199999999", icon: Phone },
  ],
};
              ...
              {footerLinks.contact.map((link) => (
                <li key={link.name}>
                  {isInternalHref(link.href) ? (
                    <Link
                      href={link.href}
                      className="flex items-center gap-2 break-all text-sm text-gray-400 transition-colors hover:text-[#CCFF00]"
                    >
                      <link.icon className="h-4 w-4 text-[#CCFF00]" />
                      {link.name}
                    </Link>
                  ) : (
                    <a
