      ...
      <DialogContent className="max-w-5xl border border-[#1D2A45] bg-[#06090F] p-0 text-white">
          ...
          <div>
            ...
            <div ref={dropdownRef} className="relative">
              <button
                ...
                disabled={isMoving}
                className="flex h-11 w-full items-center justify-between rounded-md border border-[#1D2C4A] bg-[#0A1630] px-3 text-sm outline-none focus:border-[#CCFF00] disabled:opacity-50"
              >
                <span
                  className={
                    ...
                    activeWeeks.map((week: any) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          {isSelected && (
                            <svg
                              className="ml-3 h-4 w-4 shrink-0 text-[#CCFF00]"
                              fill="none"
                              viewBox="0 0 24 24"
                              stroke="currentColor"
                              strokeWidth={2.5}
            ...
            {selectedWeeks.length > 0 && (
              <div className="mt-3 flex flex-wrap gap-2">
                {selectedWeeks.map((week: any) => (
                  <span
                    key={week.id}
                    className="flex items-center gap-1.5 rounded-full border border-[#CCFF0044] bg-[#0D2233] px-3 py-1 text-xs text-[#CCFF00]"
                  >
                    {`${week.scheduleName} — Week ${week.weekNumber}`}
                    <button
                      type="button"
        ...
        <DialogFooter className="border-t border-[#14203A] px-6 py-5 sm:justify-between">
          ...
          <button
            ...
            className="h-11 rounded-md bg-[#CCFF00] px-10 text-sm font-bold text-[#041116] hover:bg-[#2EA8B7] disabled:opacity-50"
